...
- **Tags** — `20 #food` labels a line; a line that is only `#food` (or
  ...
  tagged lines above, across the whole sheet. A row with several tags is the
  full amount split equally between them, so `burger: 2 * 50 #ana #bob` gives
  each tag 50 while the row (and the group total) keeps the full 100.
...
- **Autocomplete** — a caret-anchored popup offers the sheet's variables and
  `#tags` plus the built-in functions, keywords, constants and units as you type
  (or on `Ctrl+Space`). `↑`/`↓` choose, `Enter`/`Tab` insert, `Esc` dismisses.
...
- `js/core/` — parsing and evaluation (`calculate.js`, `parseLine.js`,
  ...
  `tabsState.js`, `history.js`, `language.js`).
